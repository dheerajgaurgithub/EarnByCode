...
export const compressionMiddleware = (req, res, next) => {
  // Skip compression for API responses that shouldn't be compressed
  if (req.path.startsWith('/api/') && req.headers['x-no-compression']) {
    return next();
  }
...
export const getRateLimitKey = (identifier, windowMs) => {
  return `rl:${identifier}:${Math.floor(Date.now() / windowMs)}`;
};
